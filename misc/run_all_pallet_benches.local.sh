...
cargo build --release --locked --features runtime-benchmarks,rococo --bin framenode

# Load all pallet names in an array.
PALLETS=($(
  ./target/release/parachain-collator benchmark pallet --list --chain="local" |\
    tail -n+2 |\
    cut -d',' -f1 |\
    sort |\
    uniq
